...
`--allow-dev-identity` 只適合本機 preview、explain、validate、dry-run。Development identity claims 不能 apply 真實 catalog 或 runtime 檔。Production apply 必須使用非 dev identity source，並搭配 repo 外受保護的 auth config。若使用 `--identity-source os-allowlist`，`admin_group` 預設為 `admin`，`[os_allowlist].users` 必須包含目前 OS user。若使用 `--identity-source verified-jwt`，需傳入 `--operator-jwt <path>`，並在 `[verified_jwt]` 設定 `issuer`、`audience`、`jwks_path`，以及選用的 `group_claim`、`max_session_age_seconds`、`clock_skew_seconds`；Apply 時會用設定的 JWKS 與 freshness window 驗證 token。Verified JWT 只接受 RSA/PSS/ECDSA/EdDSA 這類 asymmetric JWKS algorithm，會拒絕 HMAC symmetric algorithm。
