...
Use `--allow-dev-identity` only for local preview, explain, validate, and dry-run flows. Development identity claims cannot apply real catalog or runtime files. Production apply requires a non-dev identity source backed by a protected auth config outside the repository. For `--identity-source os-allowlist`, `admin_group` defaults to `admin`, and `[os_allowlist].users` must include the current OS user. For `--identity-source verified-jwt`, pass `--operator-jwt <path>` and configure `[verified_jwt]` with `issuer`, `audience`, `jwks_path`, and optional `group_claim`, `max_session_age_seconds`, and `clock_skew_seconds`; the token is verified at apply time against the configured JWKS and freshness window. Verified JWT accepts asymmetric JWKS algorithms such as RSA/PSS/ECDSA/EdDSA and rejects symmetric HMAC algorithms.
