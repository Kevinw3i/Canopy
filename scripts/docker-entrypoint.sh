...
if [ ! -f "$CONFIG_PATH" ] || [ "${GENERATE_CONFIG:-0}" = "1" ]; then
  ...
  # In-memory is the local/dev default. Multi-task deployments MUST set
  # MCP_SESSION_STORE=dynamodb (Terraform's mcp_session_store defaults to it);
  ...
  MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG=""
  if [ -n "${MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_NAME:-}" ]; then
    SAFE_MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_NAME=$(
      escape_toml "$MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_NAME"
    )
    MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG="${MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG}
ec2_diagnostic_ssm_document_name = \"${SAFE_MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_NAME}\""
  fi
  if [ -n "${MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_VERSION:-}" ]; then
    SAFE_MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_VERSION=$(
      escape_toml "$MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_VERSION"
    )
    MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG="${MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG}
ec2_diagnostic_ssm_document_version = \"${SAFE_MCP_EC2_DIAGNOSTIC_SSM_DOCUMENT_VERSION}\""
  fi
  if [ -n "${MCP_EC2_DIAGNOSTIC_HELPER_VERSION:-}" ]; then
    SAFE_MCP_EC2_DIAGNOSTIC_HELPER_VERSION=$(
      escape_toml "$MCP_EC2_DIAGNOSTIC_HELPER_VERSION"
    )
    MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG="${MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG}
ec2_diagnostic_helper_version = \"${SAFE_MCP_EC2_DIAGNOSTIC_HELPER_VERSION}\""
  fi
  if [ -n "${MCP_EC2_DIAGNOSTIC_COMMAND_SPEC_KEY:-}" ]; then
    SAFE_MCP_EC2_DIAGNOSTIC_COMMAND_SPEC_KEY=$(
      escape_toml "$MCP_EC2_DIAGNOSTIC_COMMAND_SPEC_KEY"
    )
    MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG="${MCP_EC2_DIAGNOSTIC_DISPATCH_CONFIG}
ec2_diagnostic_command_spec_key = \"${SAFE_MCP_EC2_DIAGNOSTIC_COMMAND_SPEC_KEY}\""
  fi
