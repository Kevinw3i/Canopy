...
`mfa_database_url = "sqlite:///path/to/mfa.db"` 啟用本機 MFA factor store。Control Plane 啟動時會建立 `mfa_factors` 與 `mfa_recovery_codes` schema；status endpoint 會回報 TOTP / WebAuthn factor store 是否可用、使用者是否已有 active enrolled factor，以及可用 recovery code 數量。TOTP enrollment / verify 需要 `mfa_secret_key`（base64 32 bytes，例：`openssl rand -base64 32`），secret 以 XChaCha20-Poly1305 加密後才寫入 SQLite。查詢使用 `(user_id, kind)` partial index，只掃描 `enrolled_at IS NOT NULL AND disabled_at IS NULL` 的 active factors；pending TOTP setup 使用 `(user_id, kind, created_at)` partial index 並在 confirm 時套用 10 分鐘 TTL。TOTP verify 會更新 `last_used_at` 與 `last_totp_step`，避免同一 time-step code 被重放，並建立 5 分鐘 per-JWT in-memory step-up session。Recovery codes 只能在 active TOTP factor 存在且目前 JWT 已完成 TOTP step-up 時產生或輪替；每次產生 10 組 80-bit code，SQLite 只保存 salted SHA-256 hash 與 used/disabled 狀態，明文只在 generate response 與 TUI 畫面中顯示一次。`mfa_recovery_codes` 的 active unused lookup 使用 `user_id` partial index。當使用者已註冊本機 TOTP factor 時，EC2 power 與 ECS Exec 會要求有效 step-up session，否則回傳 `403 STEP_UP_REQUIRED`。
...
- MFA 目前仍以 OIDC Provider-driven enforcement 為主；TUI 已顯示本機 MFA / step-up readiness，且在 `mfa_database_url` + `mfa_secret_key` 設定後可註冊並驗證本機 TOTP step-up、產生/輪替 recovery codes；EC2 power / ECS Exec 已支援本機 TOTP route-level step-up gate；本機 WebAuthn 註冊與 recovery-code 驗證使用尚未實作
