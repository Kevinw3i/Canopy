...
- MFA 目前仍以 OIDC Provider-driven enforcement 為主；TUI 已顯示本機 MFA / step-up readiness，且在 `mfa_database_url` + `mfa_secret_key` 設定後可註冊並驗證本機 TOTP step-up、產生/輪替與使用 recovery codes；EC2 power / ECS Exec 已支援本機 TOTP route-level step-up gate；本機 WebAuthn backend 註冊 foundation 與 TUI local browser registration flow 已實作，WebAuthn step-up 尚未實作
